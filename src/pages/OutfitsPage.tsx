            ...
            <div className="max-w-6xl mx-auto">
                ...
                <AnimatePresence>
                    {isStaging && (
                        ...
                        >
                            <div className="glass rounded-3xl p-4 border border-pink-500/30">
                                {/* Staging Header */}
                                <div className="flex items-center justify-between mb-4">
                                    <div className="flex items-center gap-2">
                                        <span className="text-xl">{OCCASION_LABELS[stagingOccasion].emoji}</span>
                                        <span className="font-bold text-white">{OCCASION_LABELS[stagingOccasion].label} Outfit</span>
                                        <span className="text-xs text-zinc-500">({stagingItems.length} items)</span>
                                    </div>
                                    <div className="flex gap-2">
                                        <button
                                            onClick={handleCancelStaging}
                                            className="px-3 py-1.5 rounded-xl text-xs font-bold bg-white/10 text-zinc-400 hover:bg-white/20 transition-colors"
                                        >
                                            Cancel
                                        </button>
                                        <button
                                            ...
                                            className="px-4 py-1.5 rounded-xl text-xs font-bold bg-gradient-to-r from-pink-500 to-orange-500 text-white disabled:opacity-50 shadow-lg"
                                        >
                                        ...
                                        return (
                                            <button
                                                ...
                                                className={`relative aspect-square rounded-xl overflow-hidden border-2 transition-all duration-200
                                                    ${isSelected
                                                        ? 'border-pink-500 ring-2 ring-pink-500/40 scale-95'
                ...
                {!isStaging && (
                    <div className="mb-8">
                        <h2 className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest mb-3 px-1">
                            Quick Start (1 tap)
                        </h2>
                        ...
                        {filteredOutfits.map((outfit) => {
                            ...
                            return (
                                <motion.div
                                    ...
                                    className="group relative glass rounded-2xl overflow-hidden hover:shadow-xl hover:shadow-pink-500/10 transition-all"
                                >
                                        ...
                                        {outfit.items && outfit.items.length > 0 ? (
                                            <div className="grid grid-cols-2 gap-0.5 p-1 h-full">
                                                ...
                                                {outfit.items.length < 4 && Array(4 - outfit.items.length).fill(0).map((_, i) => (
                                                    <div key={`empty-${i}`} className="rounded-lg bg-zinc-800/50" />
                                                ))}
                                            </div>
                                        ) : (
                                    ...
                                    <div className="p-3">
                                        ...
                                        <p className="text-xs font-bold text-white truncate">{outfit.name}</p>
                                        <p className="text-[10px] text-zinc-500">{outfit.items?.length || 0} pieces</p>
                                    </div>
                                </motion.div>
                            );
                        })}
                ...
                {!isStaging && filteredOutfits.length === 0 && !isLoading && (
                    <div className="text-center py-8">
                        ...
                        <h3 className="text-base font-bold text-white mb-1">No outfits yet</h3>
                        <p className="text-sm text-zinc-500 mb-4">Tap a vibe above to create your first outfit</p>
                    </div>
                )}
